...
import com.github.ant.rpc.{IsolatedRpcEndpoint, RpcConf, RpcEndpoint}
...
private class SharedMessageLoop(
  ...
  private def getNumOfThreads(conf: RpcConf): Int = {
    val availableCores =
      if (numUsableCores > 0) numUsableCores else Runtime.getRuntime.availableProcessors()

    val modNumThreads = conf.get("rpc.netty.dispatcher.num.threads", math.max(2, availableCores))

    conf.getOption("ant.rpc.role").map { id =>
      val role = if (id == "driver") "driver" else "executor"
      conf.getInt(s"ant.$role.rpc.netty.dispatcher.numThreads", modNumThreads)
