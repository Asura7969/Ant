...
public class TransportConf {
  ...
  private final String ANT_NETWORK_IO_MODE_KEY;
  private final String ANT_NETWORK_IO_PREFERDIRECTBUFS_KEY;
  private final String ANT_NETWORK_IO_CONNECTIONTIMEOUT_KEY;
  private final String ANT_NETWORK_IO_BACKLOG_KEY;
  private final String ANT_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY;
  private final String ANT_NETWORK_IO_SERVERTHREADS_KEY;
  private final String ANT_NETWORK_IO_CLIENTTHREADS_KEY;
  private final String ANT_NETWORK_IO_RECEIVEBUFFER_KEY;
  private final String ANT_NETWORK_IO_SENDBUFFER_KEY;
  private final String ANT_NETWORK_SASL_TIMEOUT_KEY;
  private final String ANT_NETWORK_IO_MAXRETRIES_KEY;
  private final String ANT_NETWORK_IO_RETRYWAIT_KEY;
  private final String ANT_NETWORK_IO_LAZYFD_KEY;
  private final String ANT_NETWORK_VERBOSE_METRICS;
  private final String ANT_NETWORK_IO_ENABLETCPKEEPALIVE_KEY;
  ...
  public TransportConf(String module, ConfigProvider conf) {
    this.module = module;
    this.conf = conf;
    ANT_NETWORK_IO_MODE_KEY = getConfKey("io.mode");
    ANT_NETWORK_IO_PREFERDIRECTBUFS_KEY = getConfKey("io.preferDirectBufs");
    ANT_NETWORK_IO_CONNECTIONTIMEOUT_KEY = getConfKey("io.connectionTimeout");
    ANT_NETWORK_IO_BACKLOG_KEY = getConfKey("io.backLog");
    ANT_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY =  getConfKey("io.numConnectionsPerPeer");
    ANT_NETWORK_IO_SERVERTHREADS_KEY = getConfKey("io.serverThreads");
    ANT_NETWORK_IO_CLIENTTHREADS_KEY = getConfKey("io.clientThreads");
    ANT_NETWORK_IO_RECEIVEBUFFER_KEY = getConfKey("io.receiveBuffer");
    ANT_NETWORK_IO_SENDBUFFER_KEY = getConfKey("io.sendBuffer");
    ANT_NETWORK_SASL_TIMEOUT_KEY = getConfKey("sasl.timeout");
    ANT_NETWORK_IO_MAXRETRIES_KEY = getConfKey("io.maxRetries");
    ANT_NETWORK_IO_RETRYWAIT_KEY = getConfKey("io.retryWait");
    ANT_NETWORK_IO_LAZYFD_KEY = getConfKey("io.lazyFD");
    ANT_NETWORK_VERBOSE_METRICS = getConfKey("io.enableVerboseMetrics");
    ANT_NETWORK_IO_ENABLETCPKEEPALIVE_KEY = getConfKey("io.enableTcpKeepAlive");
  }
  ...
  public String ioMode() {
    return conf.get(ANT_NETWORK_IO_MODE_KEY, "NIO").toUpperCase(Locale.ROOT);
  }

  /** If true, we will prefer allocating off-heap byte buffers within Netty. */
  public boolean preferDirectBufs() {
    return conf.getBoolean(ANT_NETWORK_IO_PREFERDIRECTBUFS_KEY, true);
  }

  /** Connect timeout in milliseconds. Default 120 secs. */
  public int connectionTimeoutMs() {
    long defaultNetworkTimeoutS = JavaUtils.timeStringAsSec(
      conf.get("ant.rpc.network.timeout", "120s"));
    long defaultTimeoutMs = JavaUtils.timeStringAsSec(
      conf.get(ANT_NETWORK_IO_CONNECTIONTIMEOUT_KEY, defaultNetworkTimeoutS + "s")) * 1000;
    return (int) defaultTimeoutMs;
  }

  /** Number of concurrent connections between two nodes for fetching data. */
  public int numConnectionsPerPeer() {
    return conf.getInt(ANT_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY, 1);
  }
   ...
   * Default to -1.
   */
  public int backLog() { return conf.getInt(ANT_NETWORK_IO_BACKLOG_KEY, -1); }

  /** Number of threads used in the server thread pool. Default to 0, which is 2x#cores. */
  public int serverThreads() { return conf.getInt(ANT_NETWORK_IO_SERVERTHREADS_KEY, 0); }

  /** Number of threads used in the client thread pool. Default to 0, which is 2x#cores. */
  public int clientThreads() { return conf.getInt(ANT_NETWORK_IO_CLIENTTHREADS_KEY, 0); }
   ...
   */
  public int receiveBuf() { return conf.getInt(ANT_NETWORK_IO_RECEIVEBUFFER_KEY, -1); }

  /** Send buffer size (SO_SNDBUF). */
  public int sendBuf() { return conf.getInt(ANT_NETWORK_IO_SENDBUFFER_KEY, -1); }

  /** Timeout for a single round trip of auth message exchange, in milliseconds. */
  public int authRTTimeoutMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get("ant.network.auth.rpcTimeout",
      conf.get(ANT_NETWORK_SASL_TIMEOUT_KEY, "30s"))) * 1000;
  }
   ...
   */
  public int maxIORetries() { return conf.getInt(ANT_NETWORK_IO_MAXRETRIES_KEY, 3); }
  ...
  public int ioRetryWaitTimeMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get(ANT_NETWORK_IO_RETRYWAIT_KEY, "5s")) * 1000;
  }
  ...
  public int memoryMapBytes() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("ant.storage.memoryMapThreshold", "2m")));
  }
  ...
  public boolean lazyFileDescriptor() {
    return conf.getBoolean(ANT_NETWORK_IO_LAZYFD_KEY, true);
  }
  ...
  public boolean verboseMetrics() {
    return conf.getBoolean(ANT_NETWORK_VERBOSE_METRICS, false);
  }
  ...
  public boolean enableTcpKeepAlive() {
    return conf.getBoolean(ANT_NETWORK_IO_ENABLETCPKEEPALIVE_KEY, false);
  }
  ...
  public int portMaxRetries() {
    return conf.getInt("ant.port.maxRetries", 16);
  }
  ...
  public boolean encryptionEnabled() {
    return conf.getBoolean("ant.network.crypto.enabled", false);
  }
  ...
  public String cipherTransformation() {
    return conf.get("ant.network.crypto.cipher", "AES/CTR/NoPadding");
  }
  ...
  public String keyFactoryAlgorithm() {
    return conf.get("ant.network.crypto.keyFactoryAlgorithm", "PBKDF2WithHmacSHA1");
  }
  ...
  public int keyFactoryIterations() {
    return conf.getInt("ant.network.crypto.keyFactoryIterations", 1024);
  }
  ...
  public int encryptionKeyLength() {
    return conf.getInt("ant.network.crypto.keyLength", 128);
  }
   ...
   */
  public int ivLength() {
    return conf.getInt("ant.network.crypto.ivLength", 16);
  }
  ...
  public String keyAlgorithm() {
    return conf.get("ant.network.crypto.keyAlgorithm", "AES");
  }
  ...
  public boolean saslFallback() {
    return conf.getBoolean("ant.network.crypto.saslFallback", true);
  }
  ...
  public boolean saslEncryption() {
    return conf.getBoolean("ant.authenticate.enableSaslEncryption", false);
  }
  ...
  public int maxSaslEncryptedBlockSize() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("ant.network.sasl.maxEncryptedBlockSize", "64k")));
  }
  ...
  public boolean saslServerAlwaysEncrypt() {
    return conf.getBoolean("ant.network.sasl.serverAlwaysEncrypt", false);
  }
  ...
  public boolean sharedByteBufAllocators() {
    return conf.getBoolean("ant.network.sharedByteBufAllocators.enabled", true);
  }
  ...
  */
  public boolean preferDirectBufsForSharedByteBufAllocators() {
    return conf.getBoolean("ant.network.io.preferDirectBufs", true);
  }
...
//    return CryptoUtils.toCryptoConf("ant.network.crypto.config.", conf.getAll());
//  }
   ...
   * retry according to the shuffle retry configs (see `ant.shuffle.io.maxRetries` and
   * `ant.shuffle.io.retryWait`), if those limits are reached the task will fail with fetch
   * failure.
   */
  public long maxChunksBeingTransferred() {
    return conf.getLong("ant.shuffle.maxChunksBeingTransferred", Long.MAX_VALUE);
  }

//  /**
//   * Percentage of io.serverThreads used by netty to process ChunkFetchRequest.
//   * When the config `ant.shuffle.server.chunkFetchHandlerThreadsPercent` is set,
...
//   * {@link org.apache.ant.network.server.TransportServer}'s default EventLoopGroup
...
//   * ant.shuffle.server.chunkFetchHandlerThreadsPercent. The returned value is rounded off to
...
//      Integer.parseInt(conf.get("ant.shuffle.server.chunkFetchHandlerThreadsPercent"));
//    int threads =
...
//  }
   ...
   * by the config `ant.shuffle.server.chunkFetchHandlerThreadsPercent` is set or not.
   */
  public boolean separateChunkFetchRequest() {
    return conf.getInt("ant.shuffle.server.chunkFetchHandlerThreadsPercent", 0) > 0;
  }
  ...
  public boolean useOldFetchProtocol() {
    return conf.getBoolean("ant.shuffle.useOldFetchProtocol", false);
  }
