...
import com.github.ant.network.protocol.message.*;
...
public final class MessageDecoder extends MessageToMessageDecoder<ByteBuf> {
  ...
  private Message decode(Message.Type msgType, ByteBuf in) {
    switch (msgType) {
      case Heartbeat:
        return Heartbeat.decode(in);

      case TaskInfo:
        return TaskInfo.decode(in);

      case TaskResult:
        return TaskResult.decode(in);

      case RegisterExecutor:
        return RegisterExecutor.decode(in);

      case VerificationTask:
        return VerificationTask.decode(in);

      case Rebalance:
        return Rebalance.decode(in);
